...
case " $* " in
  ...
  *"TestLocalLifecycleCrashMatrix"*)
    ...
    if test "$count" -eq 1; then
      ...
      printf '%s\n' 'retryable crash-matrix setup failure: bootstrap private DevPod Docker provider: transient bootstrap'
      printf '%s\n' '{"retry":"crash-matrix-bootstrap"}'
      exit 1
    fi
...
func TestVerifyRealEvidenceDoesNotRetryCrashMatrixAssertionFailure(t *testing.T) {
	root := t.TempDir()
	fakeBin := filepath.Join(root, "bin")
	if err := os.MkdirAll(fakeBin, 0o700); err != nil {
		t.Fatal(err)
...
case " $* " in
  ...
  *"TestLocalLifecycleCrashMatrix"*)
    ...
    printf '%s\n' "$count" > "$CAMP_FAKE_GO_COUNT"
    printf '%s\n' '--- FAIL: TestLocalLifecycleCrashMatrix'
    printf '%s\n' 'did not stop at pending registry forwarder'
    exit 1
    ;;
  *)
    printf 'unexpected fake go invocation: %s\n' "$*" >&2
...
`)
	...
	output, err := command.CombinedOutput()
	if err == nil {
		t.Fatalf("lifecycle evidence script succeeded unexpectedly\n%s", output)
	}
	count, err := os.ReadFile(countPath)
	if err != nil {
		t.Fatal(err)
	...
	if strings.TrimSpace(string(count)) != "1" {
		t.Fatalf("crash-matrix attempts = %q, want 1\n%s", count, output)
	}
}

func TestVerifyRealEvidenceDoesNotRetryCrashMatrixCleanupFailure(t *testing.T) {
	root := t.TempDir()
	fakeBin := filepath.Join(root, "bin")
	if err := os.MkdirAll(fakeBin, 0o700); err != nil {
		t.Fatal(err)
...
case " $* " in
  ...
  *"TestLocalLifecycleCrashMatrix"*)
    ...
    printf '%s\n' "$count" > "$CAMP_FAKE_GO_COUNT"
    printf '%s\n' '--- FAIL: TestLocalLifecycleCrashMatrix'
    printf '%s\n' 'cleanup error: close test-owned session failed'
    exit 1
    ;;
  *)
    printf 'unexpected fake go invocation: %s\n' "$*" >&2
