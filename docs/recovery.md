...
The exact executable gates are `TestLocalLifecycleVertical` and `TestLocalLifecycleCrashMatrix`. The current crash gate proves controller death after forwarder spawn and before the durable forwarding fact, followed by observation-first adoption and identity-safe teardown. It does not yet prove every process-death cut required by issue #3; the gate name is discoverable for automation, while unimplemented cuts remain an explicit acceptance blocker.
