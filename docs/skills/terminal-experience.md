...
Production `sync` and `close` select the rich lifecycle adapter only when both stdin and stdout are terminals and stdout is a true-color TTY at least `69 × 20`; JSON, CI, redirected input or output, reduced-capability, and smaller terminals retain the existing deterministic transcript. The adapter maps application progress facts directly: mirror, image inventory, archive, verified upload, pointer publication, and final cleanup. A first later-stage fact emits an explicit resumed suffix before its completion; read-only sync and already-complete sync/close use result-backed no-stage messages instead of invented waypoint completions. Close declares checkpoint stages only for a read-write non-discard run, and its typed cleanup activity can select a cleanup-only suffix without prematurely completing cleanup. Duplicate or reversed facts fail closed. Serving-refresh failure remains a terminal failure after pointer publication, and a presentation-model failure is returned to the CLI rather than being overwritten by a nil worker error. The adapter cancels and joins the worker on exit. Open, attach, reopen, recover, and status remain outside this production rich adapter until their application layers expose equivalent typed facts.

`internal/setupui/scenedump` can render deterministic lifecycle sample states (`lifecycle-progress`, `lifecycle-ready`, and `lifecycle-failure`) for review. `TestLifecycleSceneGoldens` protects exactly five ANSI scenes: progress at 80×24, 120×40, and 160×48; ready at 160×48; and failure at 120×40. Linux CLI tests exercise the production terminal probe against real PTY descriptors and prove that redirected stdin disables the rich lifecycle path. Injectable production-orchestration tests cover read-only/already-complete no-stage results, resumed upload/pointer/refresh and cleanup paths, refresh failure, model-failure propagation, non-cancelable worker joining, and a single terminal failure/recovery event without invoking real tools. No end-to-end production lifecycle PTY capture, PNG, or complete size/state matrix is claimed by this coverage; tracked production captures and exact-candidate Robot evidence remain integration gates.
