...
The reusable lifecycle scene model consumes `presentation.RichLifecycleEvent`, not subprocess output. Its event kind makes activity, resumed suffix selection, completion, terminal success, and failure distinct: activity may activate the next required stage and display its exact safe detail, but only a typed completion changes that waypoint to complete. A resumed event narrows an untouched workflow to the ordered suffix beginning at its stage; it does not mark or display omitted prefix stages as completed. Completion does not auto-activate the following stage. A failure marks the next required typed stage failed, preserves at most one safe recovery command, omits the `next:` affordance when no recovery exists, and never renders a ready band. Unknown, duplicate, or out-of-order stage identifiers fail closed as presentation protocol errors.
...
Production `sync` and `close` select the rich lifecycle adapter only when both stdin and stdout are terminals and stdout is a true-color TTY at least `69 × 20`; JSON, CI, redirected input or output, reduced-capability, and smaller terminals retain the existing deterministic transcript. The adapter maps application progress facts directly: mirror, image inventory, archive, verified upload, pointer publication, and final cleanup. A first later-stage fact emits an explicit resumed suffix before its completion; read-only sync and already-complete sync/close use result-backed no-stage messages instead of invented waypoint completions. Close declares checkpoint stages only for a read-write non-discard run, and its typed cleanup activity can select a cleanup-only suffix without prematurely completing cleanup. Duplicate or reversed facts fail closed. A serving-refresh error discovered after close cleanup is a terminal command failure while the proven `CLEANUP` waypoint remains complete. Only a typed, confirmed model-rendered rich failure suppresses `Execute`'s normal envelope: it preserves a nonzero exit without duplicating Bubble Tea's failure and recovery scene. Lifecycle composition, sprite loading, or Bubble Tea runner failures before that confirmation use the normal diagnostic envelope exactly once, including a recovery command when one is known. Plain and JSON failures retain their normal deterministic envelopes. A presentation-model failure is returned to the CLI rather than being overwritten by a nil worker error. The adapter cancels and joins the worker on exit. Open, attach, reopen, recover, and status remain outside this production rich adapter until their application layers expose equivalent typed facts.

`internal/setupui/scenedump` can render deterministic setup and lifecycle sample states through the shared `setupui.SampleFrame` dispatcher; `setupui.SampleStates` and `setupui.SampleStateUsage` are the authoritative registry and CLI help source. `TestLifecycleSceneGoldens` protects exactly five ANSI scenes: progress at 80×24, 120×40, and 160×48; ready at 160×48; and failure at 120×40. Linux CLI tests exercise the production terminal probe against real PTY descriptors and prove that redirected stdin disables the rich lifecycle path. Injectable production-orchestration tests cover read-only/already-complete no-stage results, resumed upload/pointer/refresh and cleanup paths, refresh failure after completed cleanup, model-failure propagation, non-cancelable worker joining, exactly-once rich failure rendering, and ordinary setup/sprite/runner diagnostics through `Execute` without invoking real tools. No end-to-end production lifecycle PTY capture, PNG, or complete size/state matrix is claimed by this coverage; tracked production captures and exact-candidate Robot evidence remain integration gates.
