...
Production `sync` and `close` select the rich lifecycle adapter only for human output on a true-color TTY at least `69 × 20`; JSON, CI, non-TTY, reduced-capability, and smaller terminals retain the existing deterministic transcript. The adapter maps application progress facts directly: mirror, image inventory, archive, verified upload, pointer publication, and final cleanup. It ignores supporting events rather than parsing their text, rejects mapped facts that arrive out of order, cancels and joins the worker on exit, and can render a terminal failure after all stage facts when command finalization itself fails. Open, attach, reopen, recover, and status remain outside this production rich adapter until their application layers expose equivalent typed facts.
