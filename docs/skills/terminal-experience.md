...
The interactive rich path is a single [Bubble Tea](https://charm.land) program (`internal/setupui`), one long-lived model from the first configuration prompt through completion. Setup and init provide workflow-specific fields and waypoint labels to the same `setupui.Model`; setup collects machine defaults, while init collects only the missing camp name and keeps backend/provider/context defaults out of the prompt. It follows the event-loop architecture proven by the MIT-licensed Basecamp/ONCE UI:
...
The full-screen setup workflow is first-run-only: an existing machine config bypasses its form even when no camp has been initialized. Removing the machine config restores first-run setup. Additional camps are created with `camp init [root]`, whose rich workflow remains available after machine setup and uses the same terminal-capability selection.
