...
`camp setup` is the first-run prewarm and inspection command over that same resolver. When persistent configuration is absent, human setup prompts for the non-secret source, capsule, backend, DevPod provider, and context, then delegates validation, capsule initialization, and atomic persistence to configured `init` before preparing tools. An executable already on PATH is reused only when the installer proves its authoritative binary digest. Human output reports only completed verified tool facts and the campsite; it never prints managed executable paths, checksums, or a PATH export. `camp setup --json` never prompts and retains the detailed path, managed/PATH decision, repository, version, commit, platform, locked asset digest, derived binary digest, and optional PATH export in the versioned success envelope.
