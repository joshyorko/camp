...
`camp setup` is the machine-wide prewarm and inspection command over that same resolver. When machine configuration is absent, human setup prompts only for the non-secret backend and workspace provider/context defaults, persists them atomically, and prepares tools; it never initializes or selects a camp. An executable already on PATH is reused only when the installer proves its authoritative binary digest. Human output reports only completed verified tool facts and machine readiness; it never prints managed executable paths, checksums, or a PATH export. `camp setup --json` never prompts and retains the detailed path, managed/PATH decision, repository, version, commit, platform, locked asset digest, derived binary digest, and optional PATH export in the versioned success envelope.
