...
- A terminal `camp open` constructs `devpod up --ide none --open-ide=false` with the resolved workspace root, deterministic workspace ID, configured context/provider, capsule-relative devcontainer path, and `CAMP_REGISTRY`, `CAMP_FILESERVER`, `CAMP_CAPSULE`, and `CAMP_CHECKPOINT` workspace environment. `TestExactDevPodArgv` and `TestTask3ScopedWorkspaceEnvironmentAndArgvExecution` preserve this argv contract.
