...
`lifecycle.CloseEffects` acts only on identities recorded in the session snapshot. Workspace cleanup distinguishes delete from `--keep-workspace` stop, forwarders and the supervisor use full PID/boot/start identities, services delegate to `supervisor.ServiceSupervisor.Stop`, which stops each recorded child before its helper and then proves absence, lease release uses the exact recorded revision, and materialization cleanup delegates to the ownership marker/device/inode guard. The production-composition regression in `internal/adapters/lifecycle/lifecycle_test.go` wires those concrete types and asserts child → helper → absence for both services. `lifecycle.SessionObserver` reports PID reuse explicitly and accepts live service evidence only after the existing supervisor inspector validates process topology and listeners; stopped services still require listener-absence validation. `lifecycle.ServingRefresher` accepts only the request-matching `ServingContentRefreshed` intent (or no pending intent for direct adapter use), validates both complete production service specs before stopping either service, and requires the recorded absolute Hauler command prefix `store --store <absolute-store> serve <service-name>` for `registry` or `fileserver`. It then restarts both serving roots with generation-specific launch tokens. Before the checkpoint refresh fact is appended, `CheckpointPublisher` reloads the journal, requires that exact refresh intent to be the only pending work, and records the post-refresh snapshot so new helper and child identities cannot be replaced by pre-refresh records. Refresh remains post-publication and an error remains an operational refresh failure rather than a publication rollback.
