...
The subprocess runner keeps a command with all three terminal streams in Camp's foreground process group so DevPod SSH can read and write the controlling terminal without a job-control stop. Non-interactive commands retain an isolated process group so cancellation can terminate their full subprocess tree.
