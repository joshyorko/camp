...
The shipped provider command therefore lists provider identities only; provider mutation remains absent because `app.ErrProviderMutationUnsupported` is the authoritative application contract. A standalone configuration mutation subtree also remains absent: Camp has an atomic storage primitive, but no application-owned read/update/lock use case defining redaction, concurrency, and CLI output. Do not invent either command in Cobra; add the missing application port and tests first.
